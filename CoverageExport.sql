...
DECLARE @YEARNO_16 CHAR(2); set @YEARNO_16 = '15';
...
CREATE TABLE #tempEndorseCO (
	...
	[class_oic] [char](2) NOT NULL,	[subclass_oic] [char](2) NOT NULL,
...
--WHERE pol_Yr IN ( @YEARNO_12, @YEARNO_13, @YEARNO_14, @YEARNO_15, @YEARNO_16, @YEARNO_17 )
WHERE pol_Yr BETWEEN @YEARNO_12 AND @YEARNO_18
--and pol_pre = '533'
and pol_pre IN ('506','509','510','516','536','537','540','551','552','553','555','560','567','577','580','591','592')
--and pol_pre not in ('511','515','520','533','534','535','536','539','561','562','563','564','569','573','574','578','579')
...
and h.pol_pre IN ('506','509','510','516','536','537','540','551','552','553','555','560','567','577','580','591','592')
--and h.pol_pre in ('533')

--select top 199 * from #tempPolicyCO
--where  pol_no = '000058'
--and pol_br = '502'
...
--set @YEARNO =  @YEARNO_17
--set @MonthNDayStart = '/09/01'
--set @MonthNDayEnd = '/09/30'
--set	@StartDateFrom ='20' + @YEARNO + @MonthNDayStart
--set @StartDateTo = '20' + @YEARNO + @MonthNDayEnd
...
e.flag_language , c.flag_group , e.approve_datetime ,e.endos_group, c.class_oic, c.subclass_oic
from endos e  inner join ibs_pol h  on
e.pol_Yr BETWEEN @YEARNO_12 AND @YEARNO_18 and 
...
--and h.pol_Yr IN ( @YEARNO_12, @YEARNO_13, @YEARNO_14, @YEARNO_15, @YEARNO_16, @YEARNO_17 )
and h.pol_Yr BETWEEN @YEARNO_12 AND @YEARNO_17 
--and h.pol_pre not in ('511','515','520','533','534','535','536','539','561','562','563','564','569','573','574','578','579')
--and h.pol_pre = ('533')
and h.pol_pre IN ('506','509','510','516','536','537','540','551','552','553','555','560','567','577','580','591','592')
--AND h.pol_br NOT IN ( @BranchFrom , @BranchTo )
...
ON	t.pol_yr = h.pol_yr 
	...
	--select top 1000 * from #TempCover 
...
from #tempPolicyCO tco inner join  ibs_pol h  on -- ibs_pol  (5 ) came from his_policy
...
inner join  his_cover_insurance_detail  cd on
...
--========================= For Check : Duplicate =========================
--		select PolicyNumber, CoverageCode, CoverageCode2 from #TMP_FINAL_RESULT
--		where PolicyNumber IN (select PolicyNumber from #TMP_FINAL_RESULT
--								group by PolicyNumber
--								having count(PolicyNumber) > 0)
