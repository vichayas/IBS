...
-- Chin old
--		inner join ibs_pol h  on
--		h.pol_Yr BETWEEN @YEARNO_12 AND @YEARNO_18 and 
--		e.pol_yr = h.pol_yr and e.pol_br = h.pol_br and e.app_pre = h.pol_pre and e.pol_no = h.pol_no
...
(
	...
	--SubClass =    case  h.pol_pre 
	--				when '506' then 
	--					(select top 1 case  country_code_to 
	--						when '764' then   '06' else '08' end 
	--						from his_journey_ta 
	--						where pol_yr = h.pol_yr and 
	--						pol_br = h.pol_br and 
	--						pol_pre = h.pol_pre and 
	--						pol_no = h.pol_no and 
	--						endos_seq = h.endos_seq)
	--				when '516' then 
	--					(select top 1 case  country_code_to 
	--						when '764' then   '06' else '08' end 
	--						from his_journey_ta 
...
--order by  EndorsementNumber , PolicyNumber , InsuredSeq, CoverageCode) as res
	...
	--03654-15502/POL/000167-506 that's NULL
